-- 2025-06-27: 확장된 slack-dm-sender Edge Function으로 파일 업로드 + 메시지 전송

-- 알림 함수 수정: 확장된 slack-dm-sender 사용 (메시지 + 파일 업로드)
...
DECLARE
    ...
    target_role TEXT;
BEGIN
    -- 1. 발주 요청 알림 (INSERT 시) -> middle_manager에게 (일반 메시지만)
                ...
                PERFORM net.http_post(
                    ...
                    body := jsonb_build_object(
                        ...
                        'file_upload', jsonb_build_object(
                            'order_number', NEW.purchase_order_number
                        )
                    )
                );
            END LOOP;
...
-- 변경사항:
-- 1. 확장된 slack-dm-sender Edge Function 사용
-- 2. 파일 업로드가 필요한 알림에는 'file_upload' 파라미터 추가
-- 3. 메시지 + 파일이 슬랙 메신저에서 바로 다운로드 가능
-- 4. User Token 사용으로 Bot Token 설정 불필요