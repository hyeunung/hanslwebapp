-- 2025-06-27: 알림 메시지를 웹앱 도메인 다운로드 링크로 변경 (슬랙 내 바로 다운로드)

-- 알림 함수 수정: Storage URL 대신 웹앱 도메인 다운로드 URL 사용
...
BEGIN
    ...
    IF TG_OP = 'INSERT' THEN
        ...
        -- 3. 선진행 발주서 알림 (INSERT 시 + progress_type = '선진행') -> 웹앱 도메인 다운로드
        IF NEW.progress_type = '선진행' THEN
            ...
            -- 웹앱 도메인 다운로드 URL 생성 (슬랙에서 바로 다운로드 가능)
            webapp_download_url := format(
                'https://work.hansl.com/api/download-po/%s',
    ...
    -- 4. 일반 발주서 알림 (최종승인 시 + progress_type = '일반') -> 웹앱 도메인 다운로드
...
-- 변경사항:
-- 1. Storage URL → 웹앱 도메인 URL로 변경
-- 2. https://work.hansl.com/api/download-po/{발주번호} 형식 사용
-- 3. 슬랙에서 바로 다운로드 가능 (팝업 없음)
-- 4. 동일한 도메인이라 브라우저 보안 정책에 걸리지 않음