-- 20250108a_middle_manager_insert_notification.sql
-- Purpose: Send Slack DM to middle_manager when new purchase_request is inserted.
--          Layout matches the attached image with proper approval/rejection buttons.

-- 1. Create or replace function for middle manager notifications on INSERT
...
COMMENT ON FUNCTION public.notify_middle_manager_on_insert() IS 'Send Slack DM to middle_manager when new purchase_request is inserted.';

-- 2. Create trigger for INSERT
DROP TRIGGER IF EXISTS middle_manager_insert_notify_trigger ON purchase_requests;
CREATE TRIGGER middle_manager_insert_notify_trigger
    AFTER INSERT ON purchase_requests
    FOR EACH ROW
    EXECUTE FUNCTION public.notify_middle_manager_on_insert();

COMMENT ON TRIGGER middle_manager_insert_notify_trigger ON purchase_requests IS 'Trigger to notify middle manager on new purchase request insertion.';