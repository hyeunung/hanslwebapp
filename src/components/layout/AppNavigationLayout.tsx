...
import { Building2, Home, FileText, Users, Plus, CheckSquare } from "lucide-react";
...
const navigationItems = [
  ...
  { id: 'approval', icon: CheckSquare, label: '승인관리', adminOnly: true },
...
interface AppNavigationLayoutProps {
  ...
  initialTab?: 'dashboard' | 'new' | 'list' | 'approval' | 'vendors' | 'employee';
...
export default function AppNavigationLayout({ initialTab = 'dashboard', children }: AppNavigationLayoutProps) {
  ...
  // 관리자 권한 체크
  const isAdmin = currentUserRoles.includes('app_admin') || currentUserRoles.includes('ceo') || currentUserRoles.includes('final_approver') || currentUserRoles.includes('middle_manager') || currentUserRoles.includes('hr');
