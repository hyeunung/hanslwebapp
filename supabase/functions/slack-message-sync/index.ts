...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // 1. 발주번호로 purchase_request 조회
    const { data: purchaseRequest, error: purchaseError } = await supabase
      .from('purchase_requests')
      .select('id, requester_name, middle_manager_status')
    ...
    // 2. 중간관리자 slack_id 조회
    const { data: middleManagers, error: managersError } = await supabase
      .from('employees')
      .select('slack_id')
      .contains('purchase_role', ['middle_manager'])
      .not('slack_id', 'is', null);

    if (managersError || !middleManagers?.length) {
      console.error('중간관리자 조회 실패:', managersError);
      return new Response('중간관리자를 찾을 수 없습니다', { status: 404 });
    }

    console.log('중간관리자 조회 성공:', middleManagers);

    // 3. 각 중간관리자에게 메시지 동기화 실행
...
async function syncSlackMessage(slackUserId: string, purchaseOrderNumber: string) {
  try {
    ...
    console.log(`${slackUserId}와의 메시지 동기화 시작`);
    ...
    // 5. 완료 메시지 전송
    const completionMessage = `✅ 발주번호 : ${purchaseOrderNumber} 에 대한 검증이 완료 되었습니다`;
    ...
    console.error(`${slackUserId} 메시지 동기화 실패:`, error);
  }
} 