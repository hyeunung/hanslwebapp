-- 20250108d_fix_item_count_and_add_hint.sql
-- Purpose: Fix item count display and add "나머지 품목은 웹앱에서 확인하세요" hint

-- Update function to fix item count and add webapp hint
...
BEGIN
    ...
    IF TG_OP = 'INSERT' THEN
        ...
        FOREACH current_slack_id IN ARRAY target_slack_ids LOOP
            ...
            RAISE NOTICE 'Middle manager notification sent to % (Purchase Request ID: %, Item Count: %)', current_slack_id, NEW.id, item_count;
        END LOOP;
    END IF;
    RETURN NEW;
END;
...
COMMENT ON FUNCTION public.notify_middle_manager_on_insert() IS 'Send Slack DM to middle_manager when new purchase_request is inserted (fixed item count and added webapp hint).';